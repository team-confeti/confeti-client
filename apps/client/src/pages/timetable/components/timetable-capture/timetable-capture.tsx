...
import { collectSelectedArtists } from '@pages/timetable/utils/capture';
...
const TimetableCapture = ({
  ...
  const sortedArtists = useMemo(
    () => collectSelectedArtists(boardData.stages),
    [boardData.stages],
  );
    ...
    <div ref={ref} className={styles.captureRoot}>
      ...
      <div className={styles.posterPanel}>
        ...
        <div className={styles.posterContent}>
          <div className={styles.posterTopGroup}>
            {/* Title block */}
          ...
          <div className={styles.logoArea}>{CONFETI_LOGO}</div>
        </div>
      </div>
          ...
          <div className={styles.stageHeader}>
            {boardData.stages.map((stage, idx) => (
              <Fragment key={stage.stageOrder}>
                ...
                <div className={styles.stageHeaderItem}>{stage.stageName}</div>
              </Fragment>
            ))}
          </div>
...
/**
 * 로고 SVG — 모듈 레벨 호이스팅 (정적 JSX, 매 렌더마다 재생성 방지)
 * html-to-image가 <use> sprite ref를 해석하지 못하므로 인라인 SVG 사용
 */
const CONFETI_LOGO = (
  <svg
