# 페스티벌 (Festival) — Requirements

> 실제 confeti 서비스에 게시된 페스티벌 목록을 조회하고, 공연 정보를 수정할 수 있다.

**Figma**: [Admin 명세 — 페스티벌](https://www.figma.com/design/LcVY1m4kQmMVDGaIPuiJJy/Admin-%EB%AA%85%EC%84%B8?node-id=28-2661)

---

## User Flow

1. `/festival` 접근 시 게시된 페스티벌 목록 표시
2. "진행 예정 / 진행 중" 섹션과 "종료된 공연" 섹션으로 분류
3. 각 섹션에 해당 페스티벌 갯수 표시
4. 페스티벌 카드 클릭 → 공연 정보 수정 페이지로 이동
   - 종료된 공연에서도 클릭 가능
...
| Empty   | 해당 섹션에 페스티벌 없음 | EmptyState         |
...
| PUT    | `/admin/performances/festivals`              | 페스티벌 등록/수정        | `END_POINT.PUT_FESTIVAL`                    |
| GET    | `/admin/performances/festivals/{festivalId}` | 수정할 페스티벌 단건 조회 | `END_POINT.GET_FESTIVAL_DETAIL(festivalId)` |

- Query keys: `FESTIVAL_QUERY_KEY.LIST()`, `FESTIVAL_QUERY_KEY.DETAIL(festivalId)`

---

## 구현 노트

- 페이지: `pages/festival/festival-page.tsx`
...
- [ ] 진행 예정/중인 페스티벌 목록이 카드 그리드로 표시된다
- [ ] 종료된 페스티벌 목록이 별도 섹션으로 표시된다
- [ ] 각 섹션에 페스티벌 갯수가 표시된다
